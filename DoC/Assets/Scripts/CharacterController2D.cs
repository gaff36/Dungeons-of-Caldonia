...
{
    [SerializeField] private float m_DodgeForce = 5;
    ...
    private IEnumerator dodge()
    {
        ...
        isDodging = true;
        if (m_FacingRight) gameObject.GetComponent<Rigidbody2D>().AddForce(new Vector2(m_DodgeForce, 0f), ForceMode2D.Impulse);
        else gameObject.GetComponent<Rigidbody2D>().AddForce(new Vector2(-1 * m_DodgeForce, 0f), ForceMode2D.Impulse);
